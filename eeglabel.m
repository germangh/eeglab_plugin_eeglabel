...
switch lower(type),
    ...
    case 'add',
        ...
        if ~isempty(EEG.event),
            fnames_eeg = fieldnames(EEG.event(1));
            fnames = setdiff(fnames_eeg, fieldnames(new_event_template));
            for i = 1:length(fnames),
                if isnumeric(EEG.event(1).(fnames{i})),
                    value = NaN;
                elseif ischar(EEG.event(1).(fnames{i})),
                    value = 'null';
                else
                    value = [];
                end
                new_event_template.(fnames{i}) = value;
            end
            new_eeg_event = repmat(new_event_template, length(EEG.event), 1);
            for i = 1:length(new_eeg_event),
                for j = 1:length(fnames_eeg),
                    new_eeg_event(i).(fnames_eeg{j}) = EEG.event(i).(fnames_eeg{j});
                end
            end
        end
        ...
        EEG.event = [new_eeg_event;new_event];
